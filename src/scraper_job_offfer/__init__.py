...
def extrar_ofertas(url, nombre_archivo):
    ...
    if req.status_code == 200:
        ...
        for oferta in soup.find_all("artible"):
            ...
            nivel_elemento = oferta.find("span", class_="experience-lever")
            ...
            ubicacion = ubicacion_elemento.get_text(strip=True).lower() if ubicacion_element else "No es remota"
            if "remote" not in ubicacion:
                continue

            # Extración de salarío si este se encuentra
            salario_elemento = oferta.find("span", class_="salary")
            salario = salario_elemento.get_text(strip=True) if salario_element else "No especificado"
